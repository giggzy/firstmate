...
The OpenCode plugin's `sessionOwnsLock` check walks this process's own ancestry chain to confirm it descends from the recorded lock holder before attempting an arm, so a lock file that does not trace back to this process (missing, corrupt, or held by an unrelated process) reads as read-only rather than misread as owned. An arm child that closes without an actionable watcher reason retries with backoff (`FM_WATCH_REARM_RETRY_BASE_MS`..`FM_WATCH_REARM_RETRY_MAX_MS`, up to `FM_WATCH_REARM_RETRY_LIMIT` attempts) before reporting failed.
...
| kiro-cli | Run `kiro-cli chat --list-models`, which lists the models available to the current installation and account. |
...
| Fact | Value |
|---|---|
| Busy state | Unknown: no semantic busy-state source is wired for kiro-cli (see `bin/fm-busy-lib.sh`); its rendered `Kiro is working` footer only backs the tmux delivery guards (submit acknowledgement in `bin/fm-tmux-lib.sh`, agent-process liveness in `bin/backends/tmux.sh`), never a recorded busy-state source. |
