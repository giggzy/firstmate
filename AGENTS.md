...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, and `grok`; `kiro-cli` is verified as a crewmate/scout dispatch target only, not as a primary-session harness; never dispatch on an unverified adapter.
