...
# Act on a fresh actionable transition from a push-capable backend. A
# transient fm_wake_append or fm_backend_commit_transition failure is logged
# and left for reconnect reconciliation rather than exiting: this runs inline
# in the watcher's main loop, so `exit 1` here would kill the whole watcher
# process over one transient failure instead of just this cycle.
