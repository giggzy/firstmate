...
#   /updatefirstmate, restart). A bare adapter name (claude|codex|opencode|pi|pi-signed|grok|kimi|kiro-cli)
...
if [ "$KIND" = secondmate ]; then
  case "${POS[1]:-}" in
    ''|claude|codex|opencode|pi|pi-signed|grok|kimi|kiro-cli)
      ARG3=${POS[1]:-}
      ;;
    *' '*)
...
launch_template() {
  ...
  case "$harness" in
    ...
    # Brief delivery deliberately stays plain `$(cat __BRIEF__)` rather than the
    # __OPINPUT__ encoder other adapters use: that is what was live-verified above,
    # and switching it is a separate, independently-verified change.
