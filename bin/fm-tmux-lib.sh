...
# The shared default additionally matches kiro-cli's idle input placeholder ("ask
# a question or describe a task"): it appears on the cursor/composer line when
# kiro-cli is idle and needs to be treated as "empty" (not pending input) so
# fm-send's post-Enter composer check does not false-alarm on a fast kiro-cli
# turn where the placeholder reappears before the 0.4s sleep expires. (The
# styled placeholder is dark-truecolor and so already strips to empty via
# fm_composer_strip_ghost - verified 2026-07-19 - but this literal match is a
# cheap theme-independent backstop for the plain post-Enter check, which is why
# it lives in the shared default rather than the per-harness kiro-cli regex.)
# This placeholder is intentionally NOT in fm-watch.sh's BUSY_REGEX or the
# per-harness kiro-cli regex below: the watcher checks the tail-6 lines of the
# whole pane for stale detection and the placeholder appearing there means
# kiro-cli is idle/stale (correct), not busy.
...
FM_TMUX_KIRO_CLI_BUSY_REGEX_DEFAULT='Kiro is working'
...
fm_busy_lines_match() {  # [harness]
  ...
  if [ -n "${FM_BUSY_REGEX:-}" ]; then
    ...
    case "$harness" in
      ...
      kiro-cli) regex=$FM_TMUX_KIRO_CLI_BUSY_REGEX_DEFAULT ;;
