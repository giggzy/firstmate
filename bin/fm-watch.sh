...
# without sourcing the entire watcher graph. fm-push-transition-lib.sh sources
# fm-wake-lib.sh, fm-classify-lib.sh, fm-backend.sh, and fm-transition-lib.sh in
# turn, so this remains a superset of what this watcher needs.
...
# remaining rendered-text fallback (Grok only). kiro-cli has no semantic writer
# yet (unlike its rendered-regex liveness match in bin/backends/tmux.sh and
# submit-acknowledgement footer in bin/fm-tmux-lib.sh), so a kiro-cli task
# classifies unknown here and surfaces rather than being absorbed - safe, if
# noisier, until kiro-cli gets a semantic busy-state source of its own.
