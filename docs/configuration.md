...
claude, codex, opencode, pi, and grok are all empirically verified; kiro-cli is verified as a crewmate/scout dispatch target only (not as a primary-session harness); new harnesses get verified through a supervised trial task before joining the set.
