...
| `fm-brief.sh`            | Scaffold a ship brief with a worktree-isolation assertion, a report-only scout brief with `--scout`, or a secondmate charter with `--secondmate`; no-mistakes DOD includes silence guidance for the 5–20 min document/CI steps and the amend-and-force-push recipe for "Everything up-to-date" |
