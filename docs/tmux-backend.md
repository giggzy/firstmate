...
It classifies recognized Claude, Codex, OpenCode, Pi, pi-signed, Grok, Kimi, and kiro-cli process names as `alive`, common shells as `dead`, an authoritatively absent window as `missing`, unreadable state as `unreadable`, and every other process as `ambiguous`.
...
The classifier deliberately reports `unknown`/`ambiguous` for `node`/`python`/`python3` rather than guess - per the secondmate-liveness sweep's correctness bar, a wrong `alive` is harmless but a wrong `dead` spins up a duplicate agent, so an unresolvable case must never be treated as confidently dead.
...
kiro-cli, being a native compiled binary, has no such gap.
